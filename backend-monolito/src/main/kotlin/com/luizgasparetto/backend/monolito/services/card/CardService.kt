...
@Service
class CardService(private val client: CardClient, private val mapper: ObjectMapper) {
    private val log = LoggerFactory.getLogger(CardService::class.java)

    data class CardChargeResult(val paid: Boolean, val chargeId: String?, val status: String)
    ...
    fun createOneStepCharge(
        ...
        val itemsSumCents =
                items.sumOf { (it["value"] as Number).toInt() * (it["amount"] as Number).toInt() }
        val totalCents =
                totalAmount.setScale(2, RoundingMode.HALF_UP).multiply(BigDecimal(100)).toInt()
        if (itemsSumCents + (shippingCents ?: 0) != totalCents) {
            val diff = totalCents - itemsSumCents - (shippingCents ?: 0)
            val errorMsg =
                    "CARD ONE-STEP: Validação de total falhou! " +
                            "Soma itens ($itemsSumCents¢) + frete (${shippingCents ?: 0}¢) " +
                            "!= total informado ($totalCents¢). Diferença: $diff centavos."

            // Aceita diferenças de até 1 centavo (arredondamento)
            // Rejeita diferenças maiores (erro de cálculo)
            if (Math.abs(diff) > 1) {
                log.error(errorMsg)
                throw IllegalArgumentException(errorMsg)
            } else {
                log.warn("$errorMsg (tolerado: diferença <= 1¢)")
            }
        }
        ...
        if (addShippingAsItem && (shippingCents ?: 0) > 0) {
            itemsMut += mapOf("name" to "Frete", "value" to shippingCents!!, "amount" to 1)
        }
                                ...
                                mapOf(
                                        "credit_card" to
                                                mapOf(
                                                        ...
                                                        "installments" to
                                                                installments.coerceAtLeast(1),
                                                        "customer" to
                                                                customer.filterValues { it != null }
                                                )
                                ),
        ...
        if (!addShippingAsItem && (shippingCents ?: 0) > 0) {
            body["shippings"] = listOf(mapOf("name" to "Frete", "value" to shippingCents!!))
        }
    ...
    fun cancelCharge(chargeId: String): Boolean = client.cancel(chargeId)
}
