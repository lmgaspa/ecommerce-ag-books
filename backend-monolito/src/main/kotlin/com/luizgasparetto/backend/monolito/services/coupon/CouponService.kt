...
import com.luizgasparetto.backend.monolito.extensions.normalizeCouponCode
...
class CouponService(private val couponRepository: CouponRepository) {
    ...
    fun validateCoupon(request: CouponValidationRequest): CouponValidationResult {
        // Normaliza o código recebido (remove acentos, uppercase)
        val cleanCode = request.code.normalizeCouponCode()

        log.info(
                "Validating coupon: original='{}', clean='{}', orderTotal={}",
                request.code,
                cleanCode,
                request.orderTotal
        )
        ...
        val coupon =
                couponRepository.findActiveByCodeIgnoreCase(cleanCode).orElse(null)
        ...
        try {
            ...
            if (request.orderTotal < coupon.minimumOrderValue) {
                return CouponValidationResult(
                        valid = false,
                        errorMessage =
                                "Valor mínimo do pedido não atingido. Mínimo: R$ ${coupon.minimumOrderValue}"
                )
            }
            ...
            if (usageLimit != null) {
                ...
                if (totalUsage >= usageLimit) {
                    return CouponValidationResult(valid = false, errorMessage = "Cupom esgotado")
                }
            }
            ...
            if (usageLimitPerUser != null && request.userEmail != null) {
                val userUsage =
                        couponRepository.countUsageByCouponIdAndEmail(
                                coupon.id!!,
                                request.userEmail
                        )
            ...
            if (finalTotal < BigDecimal("0.01")) {
                return CouponValidationResult(
                        valid = false,
                        errorMessage =
                                "Desconto muito alto. Valor mínimo do pedido deve ser R$ 0,01"
                )
            }

            log.info(
                    "Coupon validated successfully: code={}, discount={}, finalTotal={}",
                    request.code,
                    discountAmount,
                    finalTotal
            )
