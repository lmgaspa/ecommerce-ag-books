...
class PixCheckoutService(
    ...
    fun processCheckout(request: PixCheckoutRequest): PixCheckoutResponse {
        ...
        val expireAtInstant =
                requireNotNull(order.reserveExpiresAt) { "reserveExpiresAt nulo após reserva" }
                        .toInstant()
        pixWatcher.watch(txid, expireAtInstant)

        // 6) Sucesso total: enviar email de confirmação (PENDING/WAITING)
        runCatching { orderStatusEmailService.sendPendingEmail(order) }.onFailure { e ->
    ...
    private fun calculateOriginalTotal(request: PixCheckoutRequest): BigDecimal {
        val totalBooks =
                request.cartItems.sumOf { it.price.toBigDecimal() * BigDecimal(it.quantity) }
