...
class CardCheckoutService(
        ...
        fun processCardCheckout(request: CardCheckoutRequest): CardCheckoutResponse {
                ...
                val customer =
                        mapOf(
                                ...
                                "phone_number" to
                                        request.phone.filter { it.isDigit() }.ifBlank { null }
                        )
                ...
                if (result.chargeId.isNullOrBlank()) {
                        ...
                        return CardCheckoutResponse(
                                success = false,
                                message =
                                        "Não foi possível criar a cobrança do cartão. Tente novamente.",
                ...
                val fresh =
                        updateOrderWithChargeTx(order.id!!, result.chargeId, request.installments)
                ...
                if (result.paid && !fresh.chargeId.isNullOrBlank()) {
                        ...
                        runCatching { orderStatusEmailService.sendPendingEmail(fresh) }.onFailure {
                                e ->
                                log.warn(
                                        "Falha ao enviar email (orderId={}): {}",
                                        fresh.id,
                                        e.message
                                )
                        }
                        ...
                        return CardCheckoutResponse(
                                ...
                                securityWarningAt =
                                        60 // INVALIDAR quando faltar 60 segundos (segurança máxima)
                        )
                }
                ...
                val order =
                        orderRepository.findWithItemsById(orderId)
                                ?: throw IllegalStateException(
                                        "Order $orderId não encontrado após criação"
                                )
        ...
        private object DiscountDistributionHelper {
                ...
                fun distributeDiscount(
                        ...
                        val originalCentsTotal =
                                itemsWithCents.sumOf { it.lineTotalCents } + shippingCents
                        val discountCents =
                                discountAmount
                                        .setScale(2, RoundingMode.HALF_UP)
                                        .multiply(BigDecimal(100))
                        ...
                        val finalSumCents =
                                adjustedItems.sumOf { it.adjustedLineTotalCents } +
                                        adjustedShippingCents
                        ...
                        val adjustedItems =
                                items.mapIndexed { index, item ->
                                        ...
                                        val adjustedLineTotalCents =
                                                (item.lineTotalCents - discountApplied)
                                                        .coerceAtLeast(1)

                                        val adjustedUnitPriceCents =
                                                if (item.dto.quantity > 0) {
                                                        (adjustedLineTotalCents / item.dto.quantity)
                                                                .coerceAtLeast(1)
                                                } else {
                                                        item.unitPriceCents
                                                }
                        ...
                        if (difference != 0 && adjustedItems.isNotEmpty()) {
                                ...
                                val newUnitPrice =
                                        if (lastItem.dto.quantity > 0) {
                                                (newLineTotal / lastItem.dto.quantity)
                                                        .coerceAtLeast(1)
                                        } else {
                                ...
                                val finalItemsSum =
                                        finalAdjustedItems.sumOf { it.adjustedLineTotalCents }
